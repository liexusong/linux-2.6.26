...
struct sk_buff {
	...
	struct sock			*sk;     // 此数据包所属的socket
	ktime_t				tstamp;  // 接收此数据包的时间
	struct net_device	*dev;    // 接收此数据包的设备
	...
	unsigned int		len,         // 包的总长度
						data_len;    // 只统计分片的长度
	__u16				mac_len,     // mac头部的长度
						hdr_len;     // 头部长度
	...
	unsigned char			*head,
							*data;
	unsigned int			truesize; // skb本身的大小(包括skb结构体本身和数据)
	atomic_t				users;
};
