...
struct fn_zone {
	struct fn_zone		*fz_next;	/* Next not empty zone	*/ // 连接zone的链表
	struct hlist_head	*fz_hash;	/* Hash table pointer	*/ // fib_node哈希表
...
struct fn_hash {
	struct fn_zone	*fn_zones[33]; // 根据子掩码的长度分为32种类型
...
static int
fn_hash_lookup(struct fib_table *tb, const struct flowi *flp,
...
static void
fn_hash_select_default(struct fib_table *tb, const struct flowi *flp,
					   struct fib_result *res)
{
	int order, last_idx;
	struct hlist_node *node;
...
static int fn_hash_insert(struct fib_table *tb, struct fib_config *cfg)
{
	...
	fz = table->fn_zones[cfg->fc_dst_len]; // 根据子网长度获取对应的fn_zone对象
